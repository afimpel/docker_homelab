...
goaccess () {
    ...
    rm -v logs/goaccess/site-* >> logs/goaccess/lists-serverData.log
    more TEMP/hostsfile_domains.conf | while read -r ip hostname extras; do
        ...
        if [ "$ip" == "#" ]; then
            ...
            echo "$hostname > site-$sites_name.out" >> logs/goaccess/lists-serverData.log
            # Si hay hostnames adicionales en la misma línea
            curl --http3 -H 'Accept: application/json' -silent "https://$hostname" -o "logs/goaccess/site-${sites_name}.out"
            ...
            for extra in $extras; do
                ...
                if [ "$URLSnginx" != "nginx" ]; then
                    ...
                    curl --http3 -H 'Accept: application/json' -silent "https://$extra" -o "logs/goaccess/site-${sites_name}-${sites_name2}.out"
