...
runonce_fn () {
  ...
  if [ -f "logs/startup.pid" ]; then
    ...
    DATETIME="$(date +%Y%m%d)"
    echo -e "---\t\t ✔ \t${COMPOSE_PROJECT_NAME^^}\t | \t $(date) \t ✔ \t\t---" > logs/runonce/${DATETIME}-00_ALL.log
    find logs/runonce \( -type f -name "*-50*.log" \) -delete -printf "[ 🗑  ]\tREMOVED:\t\t  %p\n" | sort >> logs/runonce/${DATETIME}-00_ALL.log 2>&1
    echo -e " " >> logs/runonce/${DATETIME}-00_ALL.log 2>&1
    for script in config/runonce/*.sh ; do
        if [ -r "$script" ] ; then
                ...
                echo -e "----------- $(date '+%Y-%m-%d %H:%M:%S') -----------\n✔\t RUN: \tbash $script ➤" >> logs/runonce/${DATETIME}-00_ALL.log
    ...
    echo -e "---\t\t ✔ \t${COMPOSE_PROJECT_NAME^^}\t | \t $(date) \t ✔ \t\t---" >> logs/runonce/${DATETIME}-00_ALL.log
    find logs/runonce \( -type f -name "int_*.log" \) -delete -printf "[ 🗑  ]\tREMOVED:\t\t  %p\n" | sort >> logs/runonce/${DATETIME}-00_ALL.log 2>&1
    timeExec=$(diffTime "$startExec0000")
